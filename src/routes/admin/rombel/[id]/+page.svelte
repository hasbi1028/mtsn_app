<script lang="ts">
	...
	let currentRombel = $state<any>(null);
	...
	async function handleAllocate() {
		...
		const result = await allocateSiswaC({ rombelId: currentRombel.id, siswaIds: ids }) as any;
		if (result?.ok) {
	...
	async function handleRemove(siswaId: number) {
		const result = await removeSiswaC({ rombelId: currentRombel.id, siswaId }) as any;
		if (result?.ok) {
			notify.success(result.pesan);
			detailQuery.refresh();
			availableQuery.refresh();
	...
	async function handleAssignWali() {
		...
		const result = await setWaliKelasC({ rombelId: currentRombel.id, ptkId: selectedPtkId }) as any;
		if (result?.ok) {
			notify.success(result.pesan);
			waliOpen = false;
			detailQuery.refresh();
...
<a href={resolve('/admin/rombel')} class="text-sm text-muted-foreground hover:underline mb-2 inline-block">
	← Kembali
</a>
...
{:then [rombelRaw, availableSiswaRaw, allPtkRaw]}
	...
	{@const _init = (currentRombel = rombel)}
	...
	{#if !rombel}
		<PageLayout title="Rombel tidak ditemukan" description="Data rombel ini tidak ada.">
			...
			<a href={resolve('/admin/rombel')} class="inline-flex items-center gap-1 text-sm text-primary hover:underline mt-2">
		...
		>
			...
			<DataTable {columns} data={siswas} emptyMessage="Belum ada siswa di rombel ini">
				{#snippet children({ row, column })}
					{#if column.key === 'nama'}
						<a href={resolve(`/admin/siswa/${row.public_id}/profil`)} class="font-medium text-sm hover:underline">{row.nama}</a>
