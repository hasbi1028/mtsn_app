<script lang="ts">
	...
	import { getRombelListQ, getRombelStatsQ } from '$modules/rombel/rombel.remote';

	const dataQuery = Promise.all([getRombelListQ(), getRombelStatsQ()]) as Promise<[any, any]>;
...
<PageLayout title="Rombel" description="Manajemen rombongan belajar — alokasi siswa & penugasan wali kelas">
	{#await dataQuery}
		<p class="text-sm text-muted-foreground">Memuat data rombel...</p>
	{:then [rombels, stats]}
		{@const byKelas = {
		...
		{@const totalTerisi = rombels.reduce((sum: number, r: any) => sum + (r.jml_siswa || 0), 0)}
		{@const totalKapasitas = rombels.reduce((sum: number, r: any) => sum + (r.kapasitas || 40), 0)}
		{#snippet filters()}
				...
				{:else}
					<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
						{#each kelasRombels as rombel (rombel.id)}
							<a href="/admin/rombel/{rombel.public_id}" class="rounded-lg border p-3 hover:border-primary hover:bg-primary/5 transition-colors block">
