...
export function getSkmtList() {
	return db.all(sql`
		SELECT s.id, p.nama, s.instansi, s.status,
...
export function getSkbkList() {
	return db.all(sql`
		SELECT s.id, p.nama, s.instansi, s.status, s.jtm_total,
			(SELECT COUNT(*) FROM dokumen d WHERE d.ptk_id = s.ptk_id) AS ndok
...
export function getSkakptList(bulan?: string) {
	...
	if (bulan) {
		rows = db.all(sql`
			SELECT COALESCE(s.id,0) as id, p.nama, COALESCE(p.nuptk,'') AS nuptk,
	...
	return rows.map((r: any) => {
		...
		return {
			ptkId: r.id, nama: r.nama, nuptk: r.nuptk,
